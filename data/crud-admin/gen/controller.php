...
class __TABLENAME__Controller extends \Core_BackendCtl {
    ...
    public function indexAction() {
        ...
        // 列表
        $result = $__TABLENAMES__->getList($params);
        // 数据总条数
        $total = $__TABLENAMES__->getCount($params);

        // 分页url
        $url = help::url('backend/__TABLENAMES__/index').'?page=';
...
__CONTROLLER_EDIT_PRE__
            // 验证
            $result = $__TABLENAMES__->validate($pdata, 'add');
...
__CONTROLLER_ADD__
                ...
                if($result) {
                    ...
                    $this->_view->assign("errors", $__TABLENAMES__->getErrorSummary());
                }
            } else {
                // 验证失败
                $this->_view->assign('ErrorMessage', '添加失败！');
...
__CONTROLLER_EDIT_PRE__
            // 验证
            $result = $__TABLENAMES__->validate($pdata, 'edit');
            $__TABLENAMES__->parseAttributes($pdata);
