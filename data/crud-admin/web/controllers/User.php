...
class UserController extends \Core_BackendCtl {
    ...
    public function indexAction() {
        ...
        // 列表
        $result = $userModel->getLists($params);
        // 数据总条数
    ...
    public function addAction() {
        ...
        if($this->getRequest()->isPost()) {
            ...
            // 验证
            $result = $userModel->validation->validate($pdata, 'add');
            $userModel->parseAttributes($pdata);
            ...
            if($result) {
                ...
                if($result) {
                    ...
                    $this->_view->assign("errors", $userModel->validation->getErrorSummary());
                }
            } else {
                // 验证失败
                $this->_view->assign('ErrorMessage', '添加失败！');
        ...
        // 模版分配数据
        $this->_view->assign("user", $userModel);
    ...
    public function editAction() {
        ...
        if($this->getRequest()->isPost()) {
            ...
            // 验证
            $result = $userModel->validation->validate($pdata, 'edit');
            $userModel->parseAttributes($pdata);
            ...
            if($result) {
                ...
                // Model转换成数组
                $data = $userModel->toArray($pdata);
                $result = $userModel->update(array('id'=>$id), $data);
        ...
        if(!empty($id) && empty($userModel->id)) {
            $data = $userModel->select(array('where'=>array('id'=>$id)));
            $userModel->parseAttributes($data);
        }
    ...
    public function delAction() {
        ...
        // 实例化Model
        $userModel = new UserModel();
        $row = $userModel->update(array('id'=>$id), array('status'=>-1));
        if($row) {
    ...
    public function statusAction() {
        ...
        // 实例化Model
        $userModel = new UserModel();
        $row = $userModel->update(array('id'=>$id), array('status'=>$status));
        if($row) {
