...
class Rbac
{
    public function __construct($unit_test = '', $config=array())
    {
        if ((string) $unit_test === 'unit_test') {
            require_once dirname(dirname(__DIR__)) . '/tests/database/database.config';
        } if(!empty($config)) {
            global $host; $host = $config['host'];
            global $adapter; $adapter = $config['adapter'];
            global $dbname; $dbname = $config['dbname'];
            global $tablePrefix; $tablePrefix = $config['tablePrefix'];
            global $user; $user = $config['user'];
            global $pass; $pass = $config['pass'];
        } else {
